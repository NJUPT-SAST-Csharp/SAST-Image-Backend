...
internal sealed class ProcessClient(StorageOptions options) : IProcessClient
{
    ...
    )
    {
        ...
        string targetFileName = Convert.ToBase64String(Encoding.UTF8.GetBytes(newKey));
        string targetBucketName = Convert.ToBase64String(
            Encoding.UTF8.GetBytes(_options.BucketName)
        );

        const string style = "image/auto-orient,1/quality,q_50/format,webp";

        ProcessObjectRequest request = new(_options.BucketName, key)
        {
        ...
        string url = _options.GetUrl(newKey);
